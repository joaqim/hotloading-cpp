...
internal debug_read_file_result DEBUGPlatformReadEntireFile(const char *Filename) {
  ...
  Result.ContentsSize = xs_CRoundToInt(FileStatus.st_size);
...
int main() {
  ...
  auto lib = load<library>("../plugin/libmegafragma_plugin_plugin.so");
  ...
  auto gameLib = load<game_library>("../plugin/libmegafragma_Game_plugin.so");
